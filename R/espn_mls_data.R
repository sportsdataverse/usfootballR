#' Get ESPN's MLS game data (play-by-play, team and player box)
#' @author Saiem Gilani
...
#' @examples
#' \donttest{
#'   try(espn_mls_game_all(game_id = 401244185))
#' }
...
espn_mls_game_all <- function(game_id){
  ...
  play_base_url <- "http://cdn.espn.com/mls/playbyplay?render=false&userab=1&xhr=1&"
...
#'
#' \donttest{
#'   try(espn_mls_pbp(game_id = 401244185))
#' }
...
#'
#' \donttest{
#'   try(espn_mls_team_box(game_id = 401244185))
#' }
...
#' Get ESPN's MLS player box data
#' @author Saiem Gilani
#' @param game_id Game ID
#' @return Returns a player boxscore data frame
#' @keywords MLS Player Box
#' @importFrom rlang .data
#' @importFrom jsonlite fromJSON toJSON
#' @importFrom dplyr filter select rename bind_cols bind_rows
#' @importFrom tidyr unnest unnest_wider everything
...
#' @examples
#' \donttest{
#'   try(espn_mls_player_box(game_id = 401244185))
#' }
#'
espn_mls_player_box <- function(game_id){
  old <- options(list(stringsAsFactors = FALSE, scipen = 999))
  on.exit(options(old))
  play_base_url <- "http://cdn.espn.com/mls/playbyplay?render=false&userab=1&xhr=1&"
...
espn_mls_teams <- function(){
  old <- options(list(stringsAsFactors = FALSE, scipen = 999))
  on.exit(options(old))
  play_base_url <- "http://site.api.espn.com/apis/site/v2/sports/soccer/mls/teams?limit=1000"
  res <- httr::RETRY(
    "GET", play_base_url
  )
...
espn_mls_scoreboard <- function(season){
  ...
  schedule_api <- glue::glue("http://site.api.espn.com/apis/site/v2/sports/soccer/mls/scoreboard?limit=1000&dates={season_dates}")
  res <- httr::RETRY(
    "GET", schedule_api
  )
...
espn_mls_standings <- function(year){
  ...
  standings_url <- "https://site.web.api.espn.com/apis/v2/sports/soccer/mls/standings?region=us&lang=en&contentorigin=espn&type=0&level=1&sort=winpercent%3Adesc%2Cwins%3Adesc%2Cgamesbehind%3Aasc&"
  ...
  tryCatch(
    expr = {
      ...
      #creating a dataframe of the MLS raw standings table from ESPN

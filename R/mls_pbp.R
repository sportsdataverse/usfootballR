...
NULL
#' @title
#' **Load cleaned MLS play-by-play from the data repo**
#' @rdname load_mls_pbp
#' @description helper that loads multiple seasons from the data repo either into memory
#' or writes it into a db using some forwarded arguments in the dots
#' @param seasons A vector of 4-digit years associated with given MLS seasons. (Min: 2002)
#' @param ... Additional arguments passed to an underlying function that writes
#' the season data into a database (used by `update_mls_db()`).
...
#' load_mls_pbp(2021)
#' }
load_mls_pbp <- function(seasons = most_recent_mls_season(),...,
  ...
  if(isTRUE(seasons)) seasons <- 2002:most_recent_mls_season()

  stopifnot(is.numeric(seasons),
            seasons >= 2002,
            seasons <= most_recent_mls_season())

  urls <- paste0("https://raw.githubusercontent.com/saiemgilani/usfootballR-data/master/mls/pbp/rds/play_by_play_",seasons,".rds")
...
#' @name load_mls_team_box
NULL
#' @title
#' **Load cleaned MLS team box scores from the data repo**
#' @rdname load_mls_team_box
#' @description helper that loads multiple seasons from the data repo either into memory
#' or writes it into a db using some forwarded arguments in the dots
#' @param seasons A vector of 4-digit years associated with given MLS seasons. (Min: 2003)
#' @param ... Additional arguments passed to an underlying function that writes
#' the season data into a database (used by `update_mls_db()`).
#' @param dbConnection A `DBIConnection` object, as returned by [DBI::dbConnect()]
...
#' @examples
#' \donttest{
#' load_mls_team_box(2021)
#' }
load_mls_team_box <- function(seasons = most_recent_mls_season(), ...,
                              dbConnection = NULL, tablename = NULL) {
  old <- options(list(stringsAsFactors = FALSE, scipen = 999))
  on.exit(options(old))
  dots <- rlang::dots_list(...)
  ...
  if(isTRUE(seasons)) seasons <- 2003:most_recent_mls_season()

  stopifnot(is.numeric(seasons),
            seasons >= 2003,
            seasons <= most_recent_mls_season())

  urls <- paste0("https://raw.githubusercontent.com/saiemgilani/usfootballR-data/master/mls/team_box/rds/team_box_",seasons,".rds")
...
#' @name load_mls_player_box
NULL
#' @title
#' **Load cleaned MLS player box scores from the data repo**
#' @rdname load_mls_player_box
#' @description helper that loads multiple seasons from the data repo either into memory
#' or writes it into a db using some forwarded arguments in the dots
#' @param seasons A vector of 4-digit years associated with given MLS seasons. (Min: 2002)
#' @param ... Additional arguments passed to an underlying function that writes
...
#' @examples
#' \donttest{
#' load_mls_player_box(2021)
#' }
load_mls_player_box <- function(seasons = most_recent_mls_season(), ...,
                                dbConnection = NULL, tablename = NULL) {
  old <- options(list(stringsAsFactors = FALSE, scipen = 999))
  on.exit(options(old))
  dots <- rlang::dots_list(...)
  ...
  urls <- paste0("https://raw.githubusercontent.com/saiemgilani/usfootballR-data/master/mls/player_box/rds/player_box_",seasons,".rds")
...
#' @name load_mls_schedule
NULL
#' @title
#' **Load cleaned MLS schedules from the data repo**
#' @rdname load_mls_schedule
#' @description helper that loads multiple seasons from the data repo either into memory
#' or writes it into a db using some forwarded arguments in the dots
#' @param seasons A vector of 4-digit years associated with given MLS seasons. (Min: 2002)
#' @param ... Additional arguments passed to an underlying function that writes
...
#' @examples
#' \donttest{
#' load_mls_schedule(2021)
#' }
load_mls_schedule <- function(seasons = most_recent_mls_season(), ...,
                              dbConnection = NULL, tablename = NULL) {
  old <- options(list(stringsAsFactors = FALSE, scipen = 999))
  on.exit(options(old))
  dots <- rlang::dots_list(...)
  ...
  urls <- paste0("https://raw.githubusercontent.com/saiemgilani/usfootballR-data/master/mls/schedules/rds/mls_schedule_",seasons,".rds")
...
# load games file
load_mls_games <- function(){
  .url <- "https://raw.githubusercontent.com/saiemgilani/usfootballR-data/master/mls/mls_games_in_data_repo.csv"
  con <- url(.url)
...
#' @name update_mls_db
#' @aliases update_mls_db mls_db mls mls_pbp_db
#' @title
#' **Update or create a usfootballR play-by-play database**
#' @description update_mls_db() updates or creates a database with `usfootballR`
...
#' are available for `usfootballR`.
#'
#' The argument `force_rebuild` is of hybrid type. It can rebuild the play
#' by play data table either for the whole usfootballR era (with `force_rebuild = TRUE`)
...
#' @import furrr
#' @export
update_mls_db <- function(dbdir = ".",
                          dbname = "usfootballR_db",
                          tblname = "usfootballR_mls_pbp",
                          force_rebuild = FALSE,
                          db_connection = NULL) {
  old <- options(list(stringsAsFactors = FALSE, scipen = 999))
  on.exit(options(old))

  # rule_header("Update usfootballR Play-by-Play Database")
  ...
  if (!DBI::dbExistsTable(connection, tblname)) {
    build_mls_db(tblname, connection, rebuild = "NEW")
  } else if (DBI::dbExistsTable(connection, tblname) & all(force_rebuild != FALSE)) {
    build_mls_db(tblname, connection, rebuild = force_rebuild)
  }

  # get completed games
  user_message("Checking for missing completed games...", "todo")
  completed_games <- load_mls_games() %>%
    # completed games since 2002, excluding the broken games
    dplyr::filter(.data$season >= 2002) %>%
    dplyr::pull(.data$game_id)

  # function below
  missing <- get_missing_mls_games(completed_games, connection, tblname)

  # rebuild db if number of missing games is too large
  if(length(missing) > 100) {
    build_mls_db(tblname, connection, show_message = FALSE, rebuild = as.numeric(unique(stringr::str_sub(missing, 1, 4))))
    missing <- get_missing_mls_games(completed_games, connection, tblname)
  }

  # # if there's missing games, scrape and write to db
  # if (length(missing) > 0) {
  #   new_pbp <- build_usfootballR_pbp(missing, rules = FALSE)
  #
  #   if (nrow(new_pbp) == 0) {
...
# this is a helper function to build usfootballR database from Scratch
build_mls_db <- function(tblname = "usfootballR_mls_pbp", db_conn, rebuild = FALSE, show_message = TRUE) {

  old <- options(list(stringsAsFactors = FALSE, scipen = 999))
  on.exit(options(old))
  valid_seasons <- load_mls_games() %>%
    dplyr::filter(.data$season >= 2002) %>%
  ...
  if (!is.null(seasons)) {
    ...
    load_mls_pbp(seasons, dbConnection = db_conn, tablename = tblname)
  }
}

# this is a helper function to check a list of completed games
# against the games that exist in a database connection
get_missing_mls_games <- function(completed_games, dbConnection, tablename) {

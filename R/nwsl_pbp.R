...
NULL
#' @title
#' **Load cleaned NWSL play-by-play from the data repo**
#' @rdname load_nwsl_pbp
#' @description helper that loads multiple seasons from the data repo either into memory
#' or writes it into a db using some forwarded arguments in the dots
#' @param seasons A vector of 4-digit years associated with given NWSL seasons. (Min: 2002)
...
load_nwsl_pbp <- function(seasons = most_recent_nwsl_season(),...,
  ...
  if(isTRUE(seasons)) seasons <- 2002:most_recent_nwsl_season()

  stopifnot(is.numeric(seasons),
            seasons >= 2002,
            seasons <= most_recent_nwsl_season())

  urls <- paste0("https://raw.githubusercontent.com/saiemgilani/usfootballR-data/master/nwsl/pbp/rds/play_by_play_",seasons,".rds")
...
#' @name load_nwsl_team_box
NULL
#' @title
#' **Load cleaned NWSL team box scores from the data repo**
#' @rdname load_nwsl_team_box
#' @description helper that loads multiple seasons from the data repo either into memory
#' or writes it into a db using some forwarded arguments in the dots
#' @param seasons A vector of 4-digit years associated with given NWSL seasons. (Min: 2003)
#' @param ... Additional arguments passed to an underlying function that writes
#' the season data into a database (used by `update_nwsl_db()`).
#' @param dbConnection A `DBIConnection` object, as returned by [DBI::dbConnect()]
...
load_nwsl_team_box <- function(seasons = most_recent_nwsl_season(), ...,
  ...
  if(isTRUE(seasons)) seasons <- 2003:most_recent_nwsl_season()

  stopifnot(is.numeric(seasons),
            seasons >= 2003,
            seasons <= most_recent_nwsl_season())

  urls <- paste0("https://raw.githubusercontent.com/saiemgilani/usfootballR-data/master/nwsl/team_box/rds/team_box_",seasons,".rds")
...
#' @name load_nwsl_player_box
NULL
#' @title
#' **Load cleaned NWSL player box scores from the data repo**
#' @rdname load_nwsl_player_box
#' @description helper that loads multiple seasons from the data repo either into memory
#' or writes it into a db using some forwarded arguments in the dots
#' @param seasons A vector of 4-digit years associated with given NWSL seasons. (Min: 2002)
#' @param ... Additional arguments passed to an underlying function that writes
...
load_nwsl_player_box <- function(seasons = most_recent_nwsl_season(), ...,
  ...
  urls <- paste0("https://raw.githubusercontent.com/saiemgilani/usfootballR-data/master/nwsl/player_box/rds/player_box_",seasons,".rds")
...
#' @name load_nwsl_schedule
NULL
#' @title
#' **Load cleaned NWSL schedules from the data repo**
#' @rdname load_nwsl_schedule
#' @description helper that loads multiple seasons from the data repo either into memory
#' or writes it into a db using some forwarded arguments in the dots
#' @param seasons A vector of 4-digit years associated with given NWSL seasons. (Min: 2002)
#' @param ... Additional arguments passed to an underlying function that writes
...
load_nwsl_schedule <- function(seasons = most_recent_nwsl_season(), ...,
  ...
  urls <- paste0("https://raw.githubusercontent.com/saiemgilani/usfootballR-data/master/nwsl/schedules/rds/nwsl_schedule_",seasons,".rds")
...
# load games file
load_nwsl_games <- function(){
  .url <- "https://raw.githubusercontent.com/saiemgilani/usfootballR-data/master/nwsl/nwsl_games_in_data_repo.csv"
  con <- url(.url)
...
#' @name update_nwsl_db
#' @aliases update_nwsl_db nwsl_db nwsl nwsl_pbp_db
#' @title
#' **Update or create a usfootballR play-by-play database**
#' @description update_nwsl_db() updates or creates a database with `usfootballR`
...
#' @import furrr
#' @export
update_nwsl_db <- function(dbdir = ".",
                          dbname = "usfootballR_db",
                          tblname = "usfootballR_nwsl_pbp",
                          force_rebuild = FALSE,
                          db_connection = NULL) {
  old <- options(list(stringsAsFactors = FALSE, scipen = 999))
  on.exit(options(old))
  ...
  if (!DBI::dbExistsTable(connection, tblname)) {
    build_nwsl_db(tblname, connection, rebuild = "NEW")
  } else if (DBI::dbExistsTable(connection, tblname) & all(force_rebuild != FALSE)) {
    build_nwsl_db(tblname, connection, rebuild = force_rebuild)
  }

  # get completed games
  user_message("Checking for missing completed games...", "todo")
  completed_games <- load_nwsl_games() %>%
    # completed games since 2002, excluding the broken games
    dplyr::filter(.data$season >= 2002) %>%
    dplyr::pull(.data$game_id)

  # function below
  missing <- get_missing_nwsl_games(completed_games, connection, tblname)

  # rebuild db if number of missing games is too large
  if(length(missing) > 100) {
    build_nwsl_db(tblname, connection, show_message = FALSE, rebuild = as.numeric(unique(stringr::str_sub(missing, 1, 4))))
    missing <- get_missing_nwsl_games(completed_games, connection, tblname)
  }
...
build_nwsl_db <- function(tblname = "usfootballR_nwsl_pbp", db_conn, rebuild = FALSE, show_message = TRUE) {

  old <- options(list(stringsAsFactors = FALSE, scipen = 999))
  on.exit(options(old))
  valid_seasons <- load_nwsl_games() %>%
    dplyr::filter(.data$season >= 2002) %>%
  ...
  if (!is.null(seasons)) {
    ...
    load_nwsl_pbp(seasons, dbConnection = db_conn, tablename = tblname)
  }
}

# this is a helper function to check a list of completed games
# against the games that exist in a database connection
get_missing_nwsl_games <- function(completed_games, dbConnection, tablename) {
